...
- F-ORG1-19: Fixed — a venue an organizer types into the create-match form is usable for their own match immediately but carries `listed: false` and never appears in the public venue directory. Creating it now also files a pending venue registration, so it reaches the existing admin review queue and notifies the admins; approving the registration lists the venue, rejecting or suspending it de-lists it. A shadowing bug meant `created_by` recorded the venue’s own name instead of the organizer id — fixed, and the audit entry now names the organizer. `tools/rules.mjs` covers the whole path.
...
- F-ORG1-22: Fixed — demo seeding is behind the explicit demo flag (see F-XC-5), so a production build has no seeded fixtures, the app shows a demo banner whenever the flag is on, and every seeded venue and match now carries `seeded: true` so a fixture is never indistinguishable from a row a real organizer created. Covered in `tools/flows-xcut.mjs`.
